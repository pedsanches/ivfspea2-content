% test_IVFSPEA2V2_smoke.m - Integration smoke tests for IVFSPEA2V2
%
% These tests validate runtime stability of IVF/SPEA2 v2 under:
%   - AR mode (default)
%   - EAR mode (polynomial mutation on mothers)
%   - EARN mode (random mothers)
%   - N_Offspring > 1 support

function tests = test_IVFSPEA2V2_smoke
...
function setupOnce(testCase)
    ...
    verifyTrue(testCase, contains(which('IVFSPEA2V2'), paths.ivf_v2_path));
...
function testARModeCompletes(testCase)
    ...
    verifyEqual(testCase, size(Dec, 1), 20, 'Final population size must remain N');
    verifyEqual(testCase, size(Obj, 1), 20, 'Final objective matrix must contain N rows');
    ...
    verifyEqual(testCase, size(Con, 1), 20, 'Constraint matrix must contain N rows');
end
...
function [Dec, Obj, Con] = runSmallCase(algo_params)
    ...
    [Dec, Obj, Con] = platemo( ...
        'algorithm', [{@IVFSPEA2V2}, algo_params], ...
        'problem', @ZDT1, ...
        'N', 20, ...
        'maxFE', 200, ...
        'M', 2);
