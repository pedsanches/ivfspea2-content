% IVF_V4_ADAPTIVE - Ablation V2 variant: Stagnation-based trigger (H4)
%
%   KEY CHANGE: IVF activates only when the population is stagnating
%   (determined by the caller) AND the budget allows it. This concentrates
%   IVF evaluations where they are most needed rather than consuming them
%   uniformly across generations.
%
%   Change from standard IVF.m:
%     Activation trigger now requires BOTH budget check AND stagnation flag.
%     Extra parameter: is_stagnating (boolean)

function [Population, Zmin, IVF_Gen_FE, IVF_Total_FE, Mating_N] = IVF_V4_ADAPTIVE( ...
    ...
    %% V4 CHANGE: Activation requires BOTH budget AND stagnation
