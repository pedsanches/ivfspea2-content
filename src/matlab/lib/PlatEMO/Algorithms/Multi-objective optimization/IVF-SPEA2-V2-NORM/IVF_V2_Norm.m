...
%        maximum min-max normalized objective-space distance (top-3
%        candidates) with binary tournament by SPEA2 fitness.
...
function [Population, Zmin, IVF_Gen_FE, IVF_Total_FE, Mating_N] = IVF_V2_Norm( ...
    ...
    for IVF_Cycle = 1:Cycles
        ...
        for mi = 1:num_mothers
            ...
            % Compute Euclidean distance in normalized objective space
            current_pool_size = size(Father_Pool_Objs, 1);
            if current_pool_size == 0
                continue;
            end

            % Min-max normalization over the current father pool
            obj_min = min(Father_Pool_Objs, [], 1);
            obj_range = max(Father_Pool_Objs, [], 1) - obj_min;
            obj_range(obj_range < 1e-12) = 1;  % avoid division by zero on flat objectives

            norm_pool = bsxfun(@rdivide, bsxfun(@minus, Father_Pool_Objs, obj_min), obj_range);
            norm_mother = (mother_objs - obj_min) ./ obj_range;

            diffs = bsxfun(@minus, norm_pool, norm_mother);
