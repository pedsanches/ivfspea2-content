% IVF_ABL_1C - Ablation variant: top-1c father selection
%
%   Identical to the standard IVF operator except that the father is selected
%   from the top-c individuals (same pool as mothers) instead of top-2c.
%   This tests whether the relaxed selection pool is necessary.
%
%   Change from standard IVF.m:
%     Line 61: Max_Range = min(NumParents * 1, Problem.N)  [was * 2]

function [Population, Zmin, IVF_Gen_FE, IVF_Total_FE, Mating_N] = IVF_ABL_1C( ...
    ...
    % ABLATION: Select father from top-c (same pool as mothers)
    Max_Range = min(NumParents * 1, Problem.N);  % <-- CHANGED: was * 2
