% IVF_ABL_4C - Ablation variant: top-4c father selection
%
%   Identical to the standard IVF operator except that the father is selected
%   from the top-4c individuals instead of top-2c.
%   This tests whether a broader selection pool improves or degrades performance.
%
%   Change from standard IVF.m:
%     Line 61: Max_Range = min(NumParents * 4, Problem.N)  [was * 2]

function [Population, Zmin, IVF_Gen_FE, IVF_Total_FE, Mating_N] = IVF_ABL_4C( ...
    ...
    % ABLATION: Select father from top-4c (broader pool)
    Max_Range = min(NumParents * 4, Problem.N);  % <-- CHANGED: was * 2
