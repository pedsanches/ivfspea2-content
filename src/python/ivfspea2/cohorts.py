...
def build_group_coverage(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tmp = df.copy()
    tmp["Run"] = runs
    tmp["is_engineering"] = tmp["Problema"].astype(str).str.startswith(ENGINEERING_PREFIX)

    grouped = tmp.groupby(["Algoritmo", "Problema", "M", "is_engineering"], as_index=False).agg(
        n_rows=("Run", "size"),
    ...
    def classify(row: pd.Series) -> tuple[str, str]:
        ...
        ok = n_runs == 60 and run_min >= BASELINE_RUN_MIN and run_max <= BASELINE_RUN_MAX
        return expected, "ok" if ok else "mismatch"
