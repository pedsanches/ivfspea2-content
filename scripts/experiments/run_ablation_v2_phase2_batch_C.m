% run_ablation_v2_phase2_batch_C.m
% =========================================================================
% ABLATION V2 - PHASE 2 BATCH C: Configs C11-C15 (5 configs × 12 instances)
% =========================================================================
%
...
%
% Usage:
%   matlab -nodisplay -r "run('/home/pedro/desenvolvimento/ivfspea2/scripts/experiments/run_ablation_v2_phase2_batch_C.m')"
%
% =========================================================================
...
log_file = fullfile(log_dir, sprintf('ablation_v2_phase2_batchC_%s.log', datestr(now, 'yyyymmdd_HHMMSS')));
diary(log_file);
fprintf('=== ABLATION V2 PHASE 2 BATCH C - Log started: %s ===\n', datestr(now));
fprintf('MATLAB: %s\n', version);
fprintf('Log file: %s\n\n', log_file);
...
batch_workdir = fullfile(PROJECT_ROOT, 'logs', 'workdir_batchC');
...
% Phase 2 factorial combinations for this batch: C11-C15
% Each row: {name, H1, H2, H3, H4}
configs = {
    'P2_C11', 1, 1, 1, 0;   % C11: H1+H2+H3
    'P2_C12', 1, 1, 0, 1;   % C12: H1+H2+H4
    'P2_C13', 1, 0, 1, 1;   % C13: H1+H3+H4
    'P2_C14', 0, 1, 1, 1;   % C14: H2+H3+H4
    'P2_C15', 1, 1, 1, 1;   % C15: H1+H2+H3+H4 (full factorial)
};
...
job_storage = fullfile(PROJECT_ROOT, 'logs', 'parjobs_batchC');
...
fprintf('=== ABLATION V2 PHASE 2 BATCH C ===\n');
...
%% Data Inventory
fprintf('\n--- Data Inventory (Phase 2 Batch C) ---\n');
fprintf('%-50s %-10s %s\n', 'Config', 'Files', 'Status');
fprintf('%s\n', repmat('-', 1, 75));
for c = 1:size(configs, 1)
    cfg_name = configs{c,1};
...
if global_errors > 0 || global_missing_runs > 0
    error('Phase 2 Batch C failed integrity checks: %d errors, %d missing runs.', ...
        global_errors, global_missing_runs);
end
