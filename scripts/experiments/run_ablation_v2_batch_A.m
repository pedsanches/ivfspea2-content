% run_ablation_v2_batch_A.m
% =========================================================================
% ABLATION V2 - BATCH A: Baseline (B) + V1 (Dissimilar Father)
% =========================================================================
%
% This batch runs on Process 1 with 6 parpool workers.
% Configs: B (current IVF/SPEA2) and V1 (dissimilar father per mother)
% Instances: 12 representative problems (M=2 and M=3)
% Runs: 30 per config-instance pair
%
% Usage:
%   matlab -nodisplay -r "run('/home/pedro/desenvolvimento/ivfspea2/scripts/experiments/run_ablation_v2_batch_A.m')"
%
% =========================================================================
...
log_file = fullfile(log_dir, sprintf('ablation_v2_batchA_%s.log', datestr(now, 'yyyymmdd_HHMMSS')));
diary(log_file);
fprintf('=== ABLATION V2 BATCH A - Log started: %s ===\n', datestr(now));
fprintf('MATLAB: %s\n', version);
fprintf('Log file: %s\n\n', log_file);
...
problems = {
    ...
    @DTLZ4, 3;   % Spherical biased (FAILURE CASE: 10x worse median)
...
% Algorithms for this batch: B (baseline) + V1 (dissimilar father)
algorithms = {
    'IVFSPEA2',           @IVFSPEA2;           % B: current baseline
    'IVFSPEA2_V1_DISSIM', @IVFSPEA2_V1_DISSIM; % V1: dissimilar father per mother
};
