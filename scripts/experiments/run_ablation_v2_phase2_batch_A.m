% run_ablation_v2_phase2_batch_A.m
% =========================================================================
% ABLATION V2 - PHASE 2 BATCH A: Configs C0-C5 (6 configs × 12 instances)
% =========================================================================
%
% This batch runs on Process 1 with 6 parpool workers.
% 60 runs per config-instance pair.
%
% Usage:
%   matlab -nodisplay -r "run('/home/pedro/desenvolvimento/ivfspea2/scripts/experiments/run_ablation_v2_phase2_batch_A.m')"
%
% =========================================================================
...
log_file = fullfile(log_dir, sprintf('ablation_v2_phase2_batchA_%s.log', datestr(now, 'yyyymmdd_HHMMSS')));
diary(log_file);
fprintf('=== ABLATION V2 PHASE 2 BATCH A - Log started: %s ===\n', datestr(now));
fprintf('MATLAB: %s\n', version);
fprintf('Log file: %s\n\n', log_file);
...
batch_workdir = fullfile(PROJECT_ROOT, 'logs', 'workdir_batchA');
...
% Phase 2 factorial combinations for this batch: C0-C5
% Each row: {name, H1, H2, H3, H4}
configs = {
    'P2_C00', 0, 0, 0, 0;   % C0:  baseline (no factors)
    'P2_C01', 1, 0, 0, 0;   % C1:  H1 only
    'P2_C02', 0, 1, 0, 0;   % C2:  H2 only
    'P2_C03', 0, 0, 1, 0;   % C3:  H3 only
    'P2_C04', 0, 0, 0, 1;   % C4:  H4 only
    'P2_C05', 1, 1, 0, 0;   % C5:  H1+H2
};
...
job_storage = fullfile(PROJECT_ROOT, 'logs', 'parjobs_batchA');
...
fprintf('=== ABLATION V2 PHASE 2 BATCH A ===\n');
...
%% Data Inventory
fprintf('\n--- Data Inventory (Phase 2 Batch A) ---\n');
fprintf('%-50s %-10s %s\n', 'Config', 'Files', 'Status');
fprintf('%s\n', repmat('-', 1, 75));
for c = 1:size(configs, 1)
    cfg_name = configs{c,1};
...
if global_errors > 0 || global_missing_runs > 0
    error('Phase 2 Batch A failed integrity checks: %d errors, %d missing runs.', ...
        global_errors, global_missing_runs);
end
