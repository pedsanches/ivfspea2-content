%% run_ivfnsgaiii_submission.m
% Comparative run: IVF/NSGA-III vs NSGA-III
...
%   RUNNER_ID=1 matlab -batch "run('experiments/run_ivfnsgaiii_submission.m')"
%   RUNNER_ID=2 matlab -batch "run('experiments/run_ivfnsgaiii_submission.m')"
%   RUNNER_ID=3 matlab -batch "run('experiments/run_ivfnsgaiii_submission.m')"

%% Configuration
NUM_RUNS        = 30;
RUN_BASE        = 5001;  % Dedicated range: 5001..5030
MAX_FE          = 100000;
N_SAVE          = 10;
NUM_WORKERS     = 6;
NUM_RUNNERS     = 3;
SUBMISSION_TAG  = 'SUB20260330_NSGAIII';

% Algorithms: IVF/NSGA-III (AR, default params) and its base algorithm
algorithms = {
    'IVFNSGAIII', {@IVFNSGAIII, 0.10, 0.10, 5};   % ivf_rate=0.10, C=0.10, Cycles=5
    'NSGAIII',    {@NSGAIII};
};
...
log_file = fullfile(log_dir, sprintf('ivfnsgaiii_runner%d_%s.log', ...
    runner_id, datestr(now, 'yyyymmdd_HHMMSS')));
diary(log_file);

fprintf('=== IVF/NSGA-III vs NSGA-III — Runner %d/%d ===\n', runner_id, NUM_RUNNERS);
