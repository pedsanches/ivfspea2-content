...
\begin{enumerate}
	...
	\item \textbf{Mechanistic interpretation}: a geometry-dependent account of when the memetic phase helps, is neutral, or becomes detrimental under SPEA2's density-based truncation, supported by convergence trajectories over 100 evaluation checkpoints with paired per-run statistical tests and ECDF runtime analysis.
...
Beyond endpoint comparisons, we assess convergence dynamics by saving metric values at 100 evenly spaced evaluation checkpoints during each run and matching runs by ID across algorithms. This yields paired time series for 30 matched runs per instance and algorithm, enabling trajectory-level analysis and per-run paired tests. Fig.~\ref{fig:convergence-ratio} plots the median IGD ratio (base / IVF) over normalized budget for six representative instances, with interquartile bands computed from the 30 matched runs. A ratio above 1.0 indicates IVF/SPEA2 advantage at that checkpoint. FE grids that differ slightly between paired runs (due to generation-level checkpointing) are aligned by linear interpolation before ratio computation; a merge diagnostic showing that this affects only 30 of 540 run-pairs ($<$6\%) and that interpolation recovers $>$99\% of otherwise-lost data points is provided in Supplementary Information~1.

\begin{figure*}[t]
	\centering
	\includegraphics[width=0.94\textwidth]{../figures/convergence_ratio_igd.pdf}
	\caption{IGD ratio (base algorithm / IVF variant) over normalized function-evaluation budget for six representative instances. Values above 1.0 indicate IVF advantage at that checkpoint. Bands show the interquartile range across 30 matched runs. FE grids are aligned by linear interpolation; see text for details.}\label{fig:convergence-ratio}
\end{figure*}

The convergence trajectories add a temporal dimension to the endpoint analysis. On DTLZ5 and ZDT4, the IGD ratio separates above 1.0 within the first 20--30\% of the budget and remains stable thereafter, which indicates that the IVF advantage is not a late-stage artifact but an early divergence that persists. On WFG2, the ratio stays near or below 1.0 throughout, consistent with the endpoint loss. On MaF1, the advantage is modest but consistent across the budget. The ECDF analysis in Fig.~\ref{fig:ecdf-runtime} aggregates this behavior across all six instances and 10 IGD target levels: IVF/SPEA2 resolves a higher fraction of targets earlier on DTLZ5 and ZDT4 (Kolmogorov--Smirnov two-sample test on the aggregated time-to-target distributions, $p < 0.01$), while showing no detectable speed advantage on WFG and MaF instances where the final IGD comparison is also neutral or adverse.

\begin{figure*}[t]
	\centering
	\includegraphics[width=0.86\textwidth]{../figures/ecdf_runtime.pdf}
	\caption{ECDF of runtime: fraction of (instance, target, run) triples solved as a function of normalized budget fraction. Bootstrap uncertainty bands (95\%, 500 resamples) are shown for each algorithm. Kolmogorov--Smirnov test p-values for IVF vs.\ base pairs are annotated where $p < 0.05$.}\label{fig:ecdf-runtime}
\end{figure*}

To complement IGD, Fig.~\ref{fig:convergence-hv} shows the corresponding HV ratio (IVF / base) over FE. On several instances, SPEA2 maintains a persistent HV advantage throughout the budget, which is consistent with the hypothesis that IVF's local intensification trades archive diversity for proximity. This convergence--diversity trade-off is visible dynamically: it is not a late-emerging property but rather a persistent characteristic of the coupled search process.

\begin{figure*}[t]
	\centering
	\includegraphics[width=0.94\textwidth]{../figures/convergence_ratio_hv.pdf}
	\caption{HV ratio (IVF / base algorithm) over normalized function-evaluation budget for six representative instances. Values above 1.0 indicate IVF advantage in hypervolume at that checkpoint. Bands show the interquartile range across 30 matched runs.}\label{fig:convergence-hv}
\end{figure*}

To strengthen the statistical foundation beyond rank-sum tests on final values, we compute per-run summary metrics (AUC-IGD, final IGD, best IGD, and corresponding HV metrics) and apply paired Wilcoxon signed-rank tests across the 30 matched runs for each (instance, algorithm pair). After Benjamini--Hochberg FDR correction across all 108 tests (6 metrics $\times$ 6 instances $\times$ 3 algorithm pairs), 64/108 tests (59.3\%) remain significant. IVF/SPEA2 shows large effects against SPEA2 on DTLZ5 ($A_{12} < 0.25$ on AUC-IGD and best IGD), MaF1, and ZDT4. Hypervolume AUC comparisons reveal a complementary pattern: SPEA2 retains higher HV accumulation on several $M{=}3$ instances, consistent with the convergence--diversity trade-off observed in the trajectory analysis. The full table of 108 paired tests with adjusted p-values and effect sizes is provided in Supplementary Information~1.
...
The convergence trajectories in Figs.~\ref{fig:convergence-ratio}--\ref{fig:convergence-hv} add a temporal dimension to these three patterns. First, the IGD advantage on regular fronts is not a late-stage artifact: median ratio curves separate within the first 20--30\% of the budget and remain stable thereafter (Fig.~\ref{fig:convergence-ratio}). Second, the ECDF analysis (Fig.~\ref{fig:ecdf-runtime}) confirms that IVF/SPEA2 resolves more targets earlier on DTLZ5 and ZDT4 (Kolmogorov--Smirnov $p < 0.01$), while showing no detectable speed advantage on WFG and MaF instances where the final IGD comparison is also neutral or adverse. Third, the HV-ratio trajectories (Fig.~\ref{fig:convergence-hv}) show that SPEA2 maintains a persistent HV advantage on several instances throughout the entire budget, which is consistent with the hypothesis that IVF's local intensification trades archive diversity for proximity --- a trade-off that static endpoint analysis alone cannot fully resolve.
...
\begin{description}
	...
	\item[Data availability:] The processed benchmark results, statistical tables, and reproducibility artifacts supporting the findings of this study are publicly available in the IVF-SPEA2 repository on Zenodo (\url{https://doi.org/10.5281/zenodo.19071254}) \citep{zambrano2026ivfspea2repo}. Additional raw experiment outputs are available from the corresponding author upon reasonable request.
	\item[Materials availability:] Not applicable.
	\item[Code availability:] The IVF-SPEA2 source code, including the \texttt{IVFSPEA2V2} implementation, the frozen PlatEMO framework used for the experiments, MATLAB experiment runners, and the Python analysis scripts used to generate the reported tables and figures, is publicly available on Zenodo at \url{https://doi.org/10.5281/zenodo.19071254} \citep{zambrano2026ivfspea2repo}.
